...
int power(long long a, unsigned int n, int p) 
{ 
...
{ 
	...
	if (gcd(n, a) != 1) {
        cout << "gcd " << n << " and " << a << " != 1" << endl;
		return false; 
    }

	// Fermat's little theorem 
	if (power(a, n-1, n) != 1) {
        cout << "power " << a << " and " << n-1 << " and " << n << " != 1" << endl;
        cout << power(a, n-1, n) << endl;
		return false; 
    }
...
{ 
	int k = 5; 
	isPrime(49331, k)? cout << " true\n": cout << " false\n"; 
	isPrime(49367, k)? cout << " true\n": cout << " false\n"; 
    int input;
    while(cin >> input) {
        if(input == 0) break;
	    isPrime(input, k)? cout << " true\n": cout << " false\n"; 
