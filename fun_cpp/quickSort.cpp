...
struct filter {
    using result = typename detail::filter_impl<
        List, Pred, Pred::template invoke<typename List::head>::value
    >::result;
};
...
struct concat_list {
    using result = typename detail::concat_list_impl<
        typename List1::head, typename List1::tail, List2
    >::result;
};
...
private:
    ...
    using left_partition = typename filter<
        typename List::tail, scalar_compare<false, T, pivot>
    >::result;
    using right_partition = typename filter<
        typename List::tail, scalar_compare<true, T, pivot>
    >::result;
public:
    using result = typename concat_list<
        ...
        typename concat_list<
            ...
            typename quick_sort<T, right_partition>::result
        >::result
    >::result;
};
...
#define LST(x, n) list<scalar<int, x>, n>
    using input = LST(7, LST(2, LST(1, LST(8, LST(9, LST(3, LST(6, LST(5, void))))))));
#undef LST
